...
// custom mutex class using atomic 
class my_mutex {
private:
    // atomic var 
    atomic<bool> locked_flag;

public:
    my_mutex() : locked_flag(false) {}

    void lock() {
        bool expected = false;
        while (!locked_flag.compare_exchange_weak(expected, true,
...
void fatty(int id, int gluttony) {
    while (true) {
        ...
        // check if fatty self-destructed
        if (eaten[id] >= 10000) {
        ...
        // check if all fatties eaten this round
        bool all_ate = true;
        for (int i = 0; i < 3; i++) {
        ...
        // if all fatties eaten, switch turn
        if (all_ate) {
            cook_turn = true;
        }
...
int main() {
    cout << "laboratory work #5 - custom mutex implementation" << endl;
    cout << "replaced std::mutex with custom atomic-based mutex" << endl;
    cout << "each dish starts with 3000 nuggets" << endl;
    cout << "self-destruct limit: 10000 nuggets" << endl;
    cout << "simulation time: 5 seconds (5 days)" << endl;

    run_test(500, 1, "test 1: cook fired");
    run_test(1000, 10000, "test 2: no salary");
    run_test(1, 500, "test 3: cook quits");
